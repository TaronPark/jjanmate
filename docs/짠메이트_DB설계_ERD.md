...
| nickname | text | 표시 이름 |
| onboarding_niche | text | 가입 시 **직접 선택한** 니치 (자취생/저연봉근로자/무지출챌린지) |
...
니치 명칭("자취생" 등)이 아직 최종 확정 전이라는 걸 확인해서, DB 제약조건 값을 한글 이름에서 내부 고정 코드로 교체함:

| 내부 코드 (DB 저장값) | 현재 표시명 (가칭) |
|---|---|
| self_catering | 자취생 |
| low_income_worker | 저연봉근로자 |
| no_spend_challenge | 무지출챌린지 |

한글 표시명은 DB가 아니라 `niches.ts` 한 파일에서만 관리. 나중에 이름이 확정되면 이 파일의 label 값만 고치면 되고, DB 마이그레이션이나 다른 코드 수정은 필요 없음. "니치 룸" 명칭(예: "자취생 식비 절약 룸")도 이 파일의 description 필드를 조합해 생성하며 DB 구조와 무관.
...
## 5-1. 실제 검증 결과 (2026-07-20)

샌드박스가 아니라 실제 Supabase 프로젝트(`TaronPark's MVP`)에 이 스키마를 그대로 실행해서 검증 완료. 결과: 3개 테이블(profiles, posts, matching_previews), 외래키, 체크 제약조건 모두 설계대로 정상 생성됨. `reactions` 테이블은 2026-07-21 설계에 추가된 것으로 아직 실제 프로젝트에는 미적용 — 1~2주차 로그인 구현과 함께 반영 예정.

검증 과정에서 Supabase 보안 점검 도구가 "RLS(줄 단위 잠금장치)가 꺼져있다"는 경고를 띄웠고, 아직 로그인 기능도 없는 단계라 위험하지 않다고 판단해 우선 RLS만 켜두고(문 잠그기) 접근 규칙(정책)은 비워뒀음. 정책은 1~2주차 로그인 기능 구현 때 같이 설계 예정 (예: "내 글만 내가 수정 가능", "비회원도 성공 처리된 글은 읽기 가능", "반응은 본인이 남긴 것만 취소 가능" 등). **이 항목은 아직 미완료 상태.**
