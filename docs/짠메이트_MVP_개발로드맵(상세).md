...
| **[게이트] Vercel 플랜(Hobby→Pro) 업그레이드 필요성 재검토 (2026-07-25 보류 결정)** — 4주차 백그라운드 재시도 큐(`app/api/cron/retry`)를 Hobby 플랜으로 운영 중. Hobby 제약: Cron 최대 하루 1회(그보다 잦은 스케줄은 배포 자체가 실패), 프로젝트당 Cron 최대 2개, 함수 실행시간 최대 60초(Fluid Compute 미적용 기준), 스케줄된 "시" 안에서 정확한 실행 분은 Vercel이 임의로 정함. 이 때문에 `system_error` 글의 "최대 3회 재시도"가 최악의 경우 3일에 걸쳐 완료될 수 있음 — 지금은 게시량이 적어(스팟체크로 충분) 허용 가능한 리스크로 보고 그대로 감. **트래픽/게시량이 늘거나(특히 6주차 소프트 런칭 시점), 재시도 주기를 하루 1회보다 촘촘하게 가져가야 할 필요가 생기면 Pro 플랜 업그레이드(Cron 분단위 가능, 실행시간 제한 완화 등)를 반드시 재검토할 것** | 6, 7, 9 |
